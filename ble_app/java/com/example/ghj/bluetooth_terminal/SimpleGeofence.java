...
*                   validity.
    ...
    //Returns an ArrayList of the active geofences currently stored in SharedPreferences
    public static ArrayList<SimpleGeofence> getActiveGeofences(Context context) {
        ArrayList<SimpleGeofence> arrayList = new ArrayList<SimpleGeofence>();

        SimpleGeofenceStore mPrefs = new SimpleGeofenceStore(context);

        int i = 1;

        try{
            while(mPrefs.getGeofence(String.valueOf(i))!=null) {
                arrayList.add(mPrefs.getGeofence(String.valueOf(i)));
                i++;
            }

            /*
            for(int i=1;i<=mPrefs.getNextValidId();i++) {
                if (mPrefs.getGeofence(String.valueOf(i)) != null) {
                    arrayList.add(mPrefs.getGeofence(String.valueOf(i)));
                }
            }
            */
